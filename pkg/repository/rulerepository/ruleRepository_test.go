...
func Test_CreateRule_GetRule(t *testing.T) {
	...
	tests := map[string]struct {
		...
		wantValidationErr bool // if we get an error in this test we always expect it to be a validation error
	}{
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			channelNameToID := map[string]string{
				channel1.Name: channelID1,
				channel2.Name: channelID2,
			}
			tt.wantRule.Action.Channel.ID = channelNameToID[tt.wantRule.Action.Channel.Name]
			...
			existingRule.Action.Channel.ID = channelNameToID[existingRule.Action.Channel.Name]
			createdRule, err := repo.Create(ctx, existingRule)
			require.NoError(t, err)
			tt.wantRule.ID = createdRule.ID // set ID for comparison (not known beforehand)
			existingUntouchedRule.Action.Channel.ID = channelNameToID[existingUntouchedRule.Action.Channel.Name]
			_, err = repo.Create(ctx, existingUntouchedRule)
			require.NoError(t, err)

			// Update rule
			tt.rule.ID = createdRule.ID                                              // set ID for update
			tt.rule.Action.Channel.ID = channelNameToID[tt.rule.Action.Channel.Name] // set channel for update // set channel ID for update
			updatedRule, err := repo.Update(ctx, createdRule.ID, tt.rule)
			if tt.wantErr {
				require.Error(t, err)
				return
			}
