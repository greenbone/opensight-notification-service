...
const (
	...
	maxRetainedFailedSends       = 200             // buffer size for failed sends, arbitrary limit to avoid memory issues
)
...
func (s *notificationService) CreateNotification(
	...
	// only process rules after notification was sucessfully stored, this avoids forwarding
	// the notification multiple times if the client retries creating thotification
	go func() {
...
func (s *notificationService) processRules(ctx context.Context, notification models.Notification) error {
	...
	for _, action := range actions {
		sendTask := SendTask{
			ctx:          context.WithoutCancel(ctx), // individual context for each send task
...
func (s *notificationService) forwardNotification(sendTask SendTask) {
	...
	case models.ChannelTypeMail:
		recipients := strings.Split(action.Recipient, `,`) // recipient(s) can be a comma separated list
		// Convert newlines to HTML line breaks
		htmlBody := strings.ReplaceAll(body, "\n", "<br>")
		for _, recipient := range recipients {
			recipient = strings.TrimSpace(recipient)
			err := s.mailService.SendMail(ctx, channel, recipient, subject, htmlBody)
			if err != nil {
				logs.Ctx(ctx).Err(err).Int("attempt", sendTask.attempt).Msg("failed to send mail")
				sendTask.Action.Recipient = recipient // set single recipient for retry
				s.scheduleRetry(sendTask)
			}
		}
...
// Send attemps are only done periodically to save cpu load.
func (s *notificationService) forwardRetriesWorker(ctx context.Context) {
	...
	for {
		...
		case <-tick: // save cpu load by only periodically checking for failed sends
			newIndex := 0
...
func convertToMarkDownMessage(subject, body string) string {
	...
	// Step 1: Temporarily replace escaped newlines (\\n) with a placeholder
	body = strings.ReplaceAll(body, `\\n`, escapedNewlinePlaceholder)
	// Step 2: Convert unescaped literal \n strings to actual newlines
	body = strings.ReplaceAll(body, `\n`, "\n")
	// Step 3: Protect all sequences of 2+ consecutive newlines by replacing each with a placeholder
	...
	// Step 4: Now all remaining single newlines become double newlines
	body = strings.ReplaceAll(body, "\n", "\n\n")
	// Step 5: Restore consecutive newlines (each placeholder back to a newline)
	body = strings.ReplaceAll(body, consecutiveNewlinesPlaceholder, "\n")
	// Step 6: Restore escaped newlines as literal \n text
