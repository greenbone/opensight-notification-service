...
func TestCheckMattermostChannel(t *testing.T) {
	t.Run("Check mattermost channel", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("Check mattermost channel with invalid webhook URL returns an error", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("Check mattermost channel with mattermost server response 404", func(t *testing.T) {
		t.Parallel()
		...
		httpassert.New(t, router).Post("/notification-channel/mattermost/check").
			...
			Json(`{
				...
				"title": "mattermost message could not be send: http status: 404 Not Found"
			}`)
	})

	t.Run("Check mattermost channel without required url", func(t *testing.T) {
		t.Parallel()
